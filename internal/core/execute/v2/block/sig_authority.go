...
func (x AuthoritySignature) Process(batch *database.Batch, ctx *SignatureContext) (*protocol.TransactionStatus, error) {
	...
	if !ctx.parentIsSynthetic() {
		return protocol.NewErrorStatus(ctx.message.ID(), errors.BadRequest.With("a non-synthetic message cannot carry an authority signature")), nil
	}
...
func (AuthoritySignature) check(batch *database.Batch, ctx *SignatureContext, sig *protocol.AuthoritySignature) error {
	if sig.Authority == nil {
		return errors.BadRequest.With("missing authority")
	}
	if sig.TxID == nil {
...
func (x AuthoritySignature) processDirect(batch *database.Batch, ctx *SignatureContext, sig *protocol.AuthoritySignature) error {
	...
	// Queue the transaction for consideration
	// TODO Don't do this unless all authorities are satisfied
	ctx.transactionsToProcess.Add(sig.TxID.Hash())
...
func (x AuthoritySignature) processDelegated(batch *database.Batch, ctx *SignatureContext, sig *protocol.AuthoritySignature) error {
	...
	index, _, ok := signer.EntryByDelegate(sig.Authority)
	if !ok {
	...
	ok, err = ctx.Executor.AuthorityIsSatisfied(batch, ctx.transaction, status, signerAuth)
	if err != nil {
		return errors.UnknownError.Wrap(err)
	}
	if ok {
...
func (AuthoritySignature) signerIsAuthorized(batch *database.Batch, ctx *SignatureContext, sig *protocol.AuthoritySignature) error {
	// Fake a signer so we can delay changing SignerIsAuthorized
	signer := sig.Authority
	if _, err := protocol.ParseLiteIdentity(signer); err != nil {
		// If the signer isn't a LID, it must be a book so add /1 to make it a
		// page
		signer = signer.JoinPath("1")
	}
	...
	val, ok := getValidator[chain.SignerValidator](ctx.Executor, ctx.transaction.Body.Type())
	if ok {
		fallback, err := val.SignerIsAuthorized(ctx.Executor, batch, ctx.transaction, &protocol.UnknownSigner{Url: signer}, md)
		if err != nil {
			return errors.UnknownError.Wrap(err)
		}
		if !fallback {
	...
	return errors.Unauthorized.WithFormat("%v is not authorized to sign transactions for %v", sig.Authority, principal.GetUrl())
}
