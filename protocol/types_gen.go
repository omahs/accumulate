...
func (v *AuthoritySignature) MarshalBinary() ([]byte, error) {
	...
	if !(v.Vote == 0) {
		writer.WriteEnum(3, v.Vote)
	}
	if !(v.TxID == nil) {
		writer.WriteTxid(4, v.TxID)
	}
	if !(!v.Initiator) {
		writer.WriteBool(5, v.Initiator)
	}
	if !(len(v.Payments) == 0) {
...
func (v *AuthoritySignature) UnmarshalFieldsFrom(reader *encoding.Reader) error {
	...
	if x := new(VoteType); reader.ReadEnum(3, x) {
		v.Vote = *x
	}
	...
	for {
		if x := new(FeePayment); reader.ReadValue(6, x.UnmarshalBinaryFrom) {
